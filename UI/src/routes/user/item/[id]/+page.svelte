<script>
	...
	onMount(async () => {
		try {
			const res = await fetch(`${API_BASE_URL}/api/items/${itemId}`, { credentials: 'include' });
			if (res.ok) {
				...
				item = {
					...
					distance: 'Simulated',
					rating: i.rating || 4.5,
					reviews: i.reviews_count || 0,
					stock: i.stock || 0,
					icon: i.type === 'product' ? '📦' : '🔧',
					images: i.images && i.images.length > 0 ? i.images : [i.image_url || i.image || 'https://images.unsplash.com/photo-1603313011101-320f26a4f6f6?w=600&q=80'],
					description: i.description || 'No description.',
					location: `${b.city || ''}, ${b.state || ''}`,
					shopLat: b.lat || 13.087,
					shopLng: b.lng || 80.275,
					phone: b.phone || '',
					email: b.email || '',
					ownerName: b.founder?.name || b.founder_name || 'Owner'
				};
			}
		} catch (err) {
...
<div class="min-h-screen bg-gray-50 text-gray-900 transition-colors duration-300 dark:bg-gray-950 dark:text-white pb-28">
	...
	<div class="mx-auto max-w-xl space-y-5 px-4 py-6">
		...
		<div>
			<div class="flex items-start justify-between">
				<div>
					...
					<a href="/user/reviews/businesses/1" class="text-sm text-gray-500 hover:text-orange-600 hover:underline dark:text-gray-400 dark:hover:text-orange-400">{item.shop}</a>
				</div>
				{#if item.type === 'product'}
					<p class="text-2xl font-black text-orange-600 dark:text-orange-400">₹{item.price}</p>
				{/if}
			</div>
			<div class="mt-3 flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
				<span>📍 {item.distance}</span>
				<a href={`/user/reviews/items/${item.id}`} class="flex items-center gap-1 hover:text-orange-600 hover:underline dark:hover:text-orange-400">
					<span>⭐ {item.rating}</span>
					<span>({item.reviews} reviews)</span>
				</a>
				{#if item.type === 'product'}<span class="text-green-600 dark:text-green-400">✓ In Stock ({item.stock})</span
					>{/if}
			</div>
		</div>
		...
		<!-- Location -->
		<div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
			<h3 class="mb-2 font-bold">📍 Shop Location</h3>
